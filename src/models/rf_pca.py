...
from pyspark.ml.regression import RandomForestRegressor  # Updated import
...
mlflow.set_experiment("RF_experiment1")  # Update experiment name

spark = SparkSession.builder.appName("rf_model").config("spark.executor.memory", "4g").config("spark.driver.memory", "4g").config("spark.executor.cores", "2").config("spark.default.parallelism", "4").getOrCreate()
...
# Start MLflow run
with mlflow.start_run(run_name="RandomForestRegressor_PCA"):  # Update run name
    ...
    rf = RandomForestRegressor(labelCol="Impact", featuresCol="pca_features", seed=42)  # Update RandomForestRegressor parameters
    ...
    # Log parameters
    mlflow.log_params({
        "numTrees": cv_model.bestModel.stages[2].getNumTrees(),
