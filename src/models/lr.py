...
mlflow.set_experiment("LR_experiment1")

spark = SparkSession.builder.appName("lr_model").config("spark.executor.memory", "4g").config("spark.driver.memory", "4g").config("spark.executor.cores", "2").config("spark.default.parallelism", "4").getOrCreate()
...
with mlflow.start_run(run_name="LinearRegression_NoPCA"):
    ...
    # Log parameters
    mlflow.log_params({
        "regParam": model.stages[1].getRegParam(),
        "elasticNetParam": model.stages[1].getElasticNetParam()
    })
