...
# Start MLflow run
with mlflow.start_run(run_name="DecisionTree_PCA"):
    ...
    dt = DecisionTreeRegressor(labelCol="Impact", featuresCol="pca_features")

    # Create a pipeline
    pipeline = Pipeline(stages=[assembler, pca, dt])
    ...
    # Log parameters
    mlflow.log_params({
        "maxDepth": model.bestModel.stages[2].getMaxDepth()
    })
